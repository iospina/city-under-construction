...
export const AnalyticsEvents = {
  ...
  // ---- Brooklyn Mirage launch arc (added slice 2 of the launch sprint) ----
  /**
   * Fired after the share button completes either a Web Share API call
   * (mobile) or a clipboard write (desktop). Property: `parcelId`, `bbl`,
   * `outcome` ∈ 'native_share' | 'clipboard' | 'failed'.
   */
  SHARE_LINK_COPIED: 'share_link_copied',
  /**
   * Fired exactly once per parcel-detail-sheet open. Carries
   * `entry_source` ∈ 'search' | 'map' | 'share_link' | 'direct' so we can
   * separate the share-flow audience from organic engagement. Replaces
   * the older `parcel_sheet_opened_from_*` events for funnel work; the
   * older events remain for backward compatibility with existing dashboards.
   */
  PARCEL_DETAIL_VIEWED: 'parcel_detail_viewed',
  /**
   * Fired exactly once per cold-start session, the first time a parcel
   * detail renders. Property: `ms` (ms since navigation start),
   * `entry_source`. The Aha moment's sub-5-second target is measured here.
   */
